...
def register_main_routes(app, safe_name, build_tournament_info, build_cards, first_image, count_images, copy_unique, dropbox_zip_path, process_mobile_job):
    ...
    def tournaments_page():
        ...
        return render_template("index.html", tournaments=tournaments, processor_available=ctx.PROCESSOR_AVAILABLE)
    ...
    def tournament(name):
        ...
        return render_template("tournament.html", tournament=name, cards=build_cards(name), export_ready=os.path.exists(dropbox_zip_path(name)))
    ...
    def gallery(tournament, folder):
        ...
        for item in sorted(os.listdir(folder_path)):
            ...
            if os.path.isdir(full):
                subfolders.append({"name":item,"count":count_images(full),"preview":first_image(full),"url":f"/gallery/{tournament}/{folder}/{item}"})
            elif item.lower().endswith(ctx.IMAGE_EXTENSIONS):
                images.append(item)
        return render_template("gallery.html", tournament=tournament, folder=folder, subfolders=subfolders, images=images)
    ...
    def photo_view(tournament, folder):
        ...
        folder_path=os.path.join(ctx.TOURNAMENT_DIR,tournament,folder)
        if not os.path.exists(folder_path): abort(404)
        images=[f for f in sorted(os.listdir(folder_path)) if f.lower().endswith(ctx.IMAGE_EXTENSIONS)]
        if not filename or filename not in images: abort(404)
        i=images.index(filename)
        fav=os.path.join(ctx.TOURNAMENT_DIR,tournament,"Favorites",filename)
        return render_template("photo.html", tournament=tournament, folder=folder, filename=filename, index=i, total=len(images), prev_file=images[i-1] if i>0 else None, next_file=images[i+1] if i<len(images)-1 else None, is_favorite=os.path.exists(fav))
    ...
    def favorite_photo(tournament, folder, filename):
        ...
        if not os.path.exists(source): abort(404)
        copy_unique(source, os.path.join(ctx.TOURNAMENT_DIR,tournament,"Favorites"))
        return redirect(url_for("photo_view", tournament=tournament, folder=folder, file=filename))

    @app.route("/image/<tournament>/<path:folder>/<filename>")
    def image(tournament, folder, filename):
        return send_from_directory(os.path.join(ctx.TOURNAMENT_DIR,tournament,folder), filename)
        ...
        if os.path.exists(unk):
            for f in sorted(os.listdir(unk)):
                if f.lower().endswith(ctx.IMAGE_EXTENSIONS):
                    items.append({"folder":"Players/Unknown","file":f,"reason":"Unknown player","ocr_number":""})

        report=os.path.join(ctx.TOURNAMENT_DIR,tournament,"Reports","diamondvision_report.csv")
        if os.path.exists(report):
            import csv
            with open(report,newline="") as fh:
                for row in csv.DictReader(fh):
                    try: conf=float(row.get("OCR Confidence",0) or 0)
                    except Exception: conf=0
                    pp=row.get("Player Path","")
                    if pp and conf>0 and conf<0.50:
                        rel=os.path.relpath(pp,os.path.join(ctx.TOURNAMENT_DIR,tournament)).replace("\\","/")
                        folder="/".join(rel.split("/")[:-1]); file=rel.split("/")[-1]
                        if not any(x["folder"]==folder and x["file"]==file for x in items):
                            items.append({"folder":folder,"file":file,"reason":f"Low OCR confidence: {conf:.2f}","ocr_number":row.get("OCR Number","")})

        players=[]
        root=os.path.join(ctx.TOURNAMENT_DIR,tournament,"Players")
        if os.path.exists(root):
            for item in sorted(os.listdir(root)):
                if os.path.isdir(os.path.join(root,item)): players.append(item)
                ...
                if os.path.exists(pr):
                    for player in sorted(os.listdir(pr)):
                        if os.path.isdir(os.path.join(pr,player)):
                            ...
                            if opt not in players: players.append(opt)

        groups={}
        for item in items:
            key=f"Likely #{item.get('ocr_number')}" if item.get("ocr_number") else item.get("reason","Review")
            if "Low OCR confidence" in key: key="Low-confidence OCR"
            if key=="Unknown player": key="Unknown player"
            groups.setdefault(key,[]).append(item)

        return render_template("review.html", tournament=tournament, items=items, groups=groups, players=players)
    ...
    def review_move(tournament, folder, filename):
        ...
        if not target_player: return redirect(url_for("review_queue",tournament=tournament))
        source=os.path.join(ctx.TOURNAMENT_DIR,tournament,folder,filename)
        if not os.path.exists(source): abort(404)
    ...
    def review_group_move(tournament):
        ...
        if not target_player or not files: return redirect(url_for("review_queue",tournament=tournament))
        dest_folder=target_folder_for(tournament,target_player)
        os.makedirs(dest_folder,exist_ok=True)
        for item in files:
            ...
            if os.path.exists(source):
                ...
                if os.path.exists(dest):
                    n,e=os.path.splitext(filename); dest=os.path.join(dest_folder,f"{n}_{datetime.now().strftime('%H%M%S%f')}{e}")
    ...
    def live_upload(tournament):
        ...
        if not os.path.exists(tpath): abort(404)
        live_dir=os.path.join(ctx.UPLOAD_DIR,"LIVE_"+tournament); os.makedirs(live_dir,exist_ok=True)
        log=os.path.join(tpath,"live_processed_files.txt")
        done=set(open(log,encoding="utf-8").read().splitlines()) if os.path.exists(log) else set()

        if request.method=="POST":
            stamp=datetime.now().strftime("%Y-%m-%d-%H%M%S")
            batch=os.path.join(live_dir,stamp); os.makedirs(batch,exist_ok=True)
            saved=0; skipped=0
            for f in request.files.getlist("photos"):
                if not f or not f.filename: continue
                name=secure_filename(f.filename); ext=os.path.splitext(name)[1].lower()
                if ext not in ctx.UPLOAD_EXTENSIONS: continue
                if name in done: skipped+=1; continue
                f.save(os.path.join(batch,name)); done.add(name); saved+=1
            with open(log,"w",encoding="utf-8") as out:
                for name in sorted(done): out.write(name+"\n")
            if saved>0 and ctx.PROCESSOR_AVAILABLE:
                ...
                ctx.JOB_STATUS[job]={"done":0,"total":saved,"percent":0,"message":"Live upload queued","complete":False,"error":""}

                def progress(d,total,msg):
                    ctx.JOB_STATUS[job]={"done":d,"total":total,"percent":int((d/total)*100) if total else 0,"message":msg,"complete":d>=total and total>0,"error":""}

                def runner():
                    try:
                        process_mobile_job(batch,tpath,roster,progress_callback=progress)
                        ctx.JOB_STATUS[job]["complete"]=True; ctx.JOB_STATUS[job]["percent"]=100; ctx.JOB_STATUS[job]["message"]="Processing complete"
                    except Exception as e:
                        ctx.JOB_STATUS[job]={"done":0,"total":0,"percent":0,"message":"Error","complete":True,"error":str(e)}
