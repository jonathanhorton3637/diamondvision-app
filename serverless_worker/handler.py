...
def log(message):
    print(f"[DiamondVision Worker] {message}", flush=True)
...
def dropbox_download(dbx, dropbox_path, local_path):
    ...
    log(f"Dropbox download complete: {local_path}")
...
def handler(job):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        jpg_outputs = [f for f in output_files if f.lower().endswith((".jpg", ".jpeg"))]
